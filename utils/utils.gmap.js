/*
	...
	@map_marker - (optional) defines the type of marker to use --> defaults to default gmap marker
...
const GMAP = class extends UTILS.Base {
	...
	getDefaults(){
		return {
			...
			is_shown: false,
			google_api_key: '', //holds the google api key
			gmap: {
	...
	getDefaultConfig(config){
		var defaults = {
			...
			marker: {
				$elm: $('<div id="gmap-marker-'+this.getId()+'" class="gmap-marker">'),
				$content: $('<div class="gmap-content">'),
				$img: $('<img>'),
				$header: $('<h2 class="gmap-marker-header">'),
				$body: $('<div class="gmap-marker-body">'),
				$shadow: $('<div class="gmap-shadow">'),
				...
				contact: {
					img: '/images/icons/gmap-marker-340x160.jpg'
				}
			}
		};
	...
	getSettings(){
		return this.values.gmap.config.settings;
	}
	...
	getStyles(){
		return this.values.gmap.config.styles;
	}
	setStyles(styles){
		_.isArray(styles) && (this.values.gmap.config.styles = styles);
		return this;
	}
				...
				geocoder.geocode({ address:this.getAddress() },function(results,status){
					if (status==google.maps.GeocoderStatus.OK){
						...
						if (marker.is_special_marker)
							var coords = new google.maps.LatLng(loc.lat()-0.00096,loc.lng()+0.0015); //lat - up/down, lng - left/right ... from the center
						else
		...
		if (content && type){
			switch(type){
				...
				case 'address':
					...
					break;
				case 'directions': html = '<span><a href="http://maps.google.com/maps?q='+UTILS.format.urlEncode(content)+'" target="_blank"><i class="class="mdi mdi-map-marker"></i> Get Directions</a></span>'; break;
			}
		}
