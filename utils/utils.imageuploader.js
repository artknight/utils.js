...
(function (window, $, undefined) {
	...
	$.imageUploader = function imageUploader(options, element) {

		this.element		= element;
		this.options		= $.extend(true, {}, $.imageUploader.defaults, options, $(this.element).data());
		...
		//buttons
		this.button				= {}
		this.button.edit		= '<div class="btn btn-info btn-edit" title="' + (this.options.editTitle || 'Edit') + '"><i class="mdi mdi-pencil"></i></div>'
		this.button.saving		= '<div class="btn btn-warning saving">' + (this.options.saveLabel || 'Saving...') + ' <i class="mdi mdi-clock"></i></div>';
		this.button.zoomin		= '<div class="btn btn-light btn-zoom-in" title="' + (this.options.zoominTitle || 'Zoom in') + '"><i class="mdi mdi-magnify-plus"></i></div>';
		this.button.zoomout		= '<div class="btn btn-light btn-zoom-out" title="' + (this.options.zoomoutTitle || 'Zoom out') + '"><i class="mdi mdi-magnify-minus"></i></div>';
		this.button.zoomreset	= '<div class="btn btn-light btn-zoom-reset" title="' + (this.options.zoomresetTitle || 'Fullsize') + '"><i class="mdi mdi-fullscreen"></i></div>';
		//this.button.rotatecw	= '<div class="btn btn-light btn-rotate-cw" title="' + (this.options.cwTitle || 'Rotate clockwise') + '"><i class="mdi mdi-rotate-right"></i></div>';
		//this.button.rotateccw	= '<div class="btn btn-light btn-rotate-ccw" title="' + (this.options.ccwTitle || 'Rotate counter clockwise') + '"><i class="mdi mdi-rotate-left"></i></div>';
		this.button.cancel		= '<div class="btn btn-danger btn-cancel" title="' + (this.options.cancelTitle || 'Cancel') + '"><i class="mdi mdi-cancel"></i></div>';
		this.button.done		= '<div class="btn btn-success btn-ok" title="' + (this.options.okTitle || 'Ok') + '"><i class="mdi mdi-check"></i></div>';
		this.button.del			= '<div class="btn btn-danger btn-del" title="' + (this.options.delTitle || 'Delete') + '"><i class="mdi mdi-delete"></i></div>';

		this.button.download	= '<a class="btn btn-warning download"><i class="mdi mdi-download"></i> ' + (this.options.downloadLabel || 'Download') + '</a>';
	...
	$.imageUploader.prototype = {
		_init: function() {
			...
			if (! (window.FormData && ("upload" in ($.ajaxSettings.xhr())))) {
				$(element).empty().attr('class','').addClass('alert alert-danger').html('Upload Image: Sadly.. this browser does not support the plugin, update your browser today!');
				return false;
			}

			if (!empty(options.width) && empty(options.height) && $(element).innerHeight() <= 0) {
				$(element).empty().attr('class','').addClass('alert alert-danger').html('Upload Image: Image height is not set and can not be calculated...');
				return false;
			}
			if (!empty(options.height) && empty(options.width) && $(element).innerWidth() <= 0) {
				$(element).empty().attr('class','').addClass('alert alert-danger').html('Upload Image: Image width is not set and can not be calculated...');
				return false;
			}
			/*if (!empty(options.height) && !empty(options.width) && !empty($(element).innerHeight() && !empty($(element).innerWidth()))) {
				...
				if ((options.width / options.height) != ($(element).outerWidth() / $(element).outerHeight())) {
					$(element).empty().attr('class','').addClass('alert alert-danger').html('Upload Image: The ratio of all sizes (CSS and image) are not the same...');
					return false;
				}
			}*/
			...
			if (!empty(options.image) && options.editstart == false) {
				...
				_self.toggleDropzoneClass(true);
		...
		toggleDropzoneClass: function(state){
			var element = this.element;
			$(element).toggleClass('no-image',state||false);
		},
		...
		imageHandle: function(e) {
			...
			$(document).on({'dragstart mousemove touchmove': function(e) {
		...
		_ajax: function(obj) {
			...
			$.ajax({
				...
				error: function(response, status) {
					...
					_self.onAjaxError(response,_self.responseReset);
				}
			})
		},
		imageReset: function() {
		...
		imageFinal: function() {
			...
			if (_self.options.onAfterUploadImage) _self.options.onAfterUploadImage.call(_self,_self);
		},
		...
		onAjaxError: function(response,callback){
		...
		imageRemove: function(){
			var _self = this;
			if (_.isPlainObject(_self.options.removeurl) && ('url' in _self.options.removeurl)){
				$.ajax({
					type: 'POST',
		...
		reset: function() {
			...
			//if image actually exists, make the call --> $(element).data('name') will only be not null if it got uploaded or existed when the page was loaded
			$(element).data('name') && _self.imageRemove();
			...
			//if options.onAfterCancel exists
			(_self.options.onAfterCancel) && _self.options.onAfterCancel.call(_self);
		},
		resize: function() {
			var _self			= this;
			var options			= _self.options;
	...
	$.fn.imageUploader = function (options) {
		if ($.data(this, "image-uploader")) return;
		return $(this).each(function() {
			$.data(this, "image-uploader",new $.imageUploader(options, this));
		})
	}
